...
Receptor = readmatrix("single_pass_library.txt", opts);
clear opts;

SHORT = [];
for i = 1:numel(Receptor)
    if strcmp(Receptor{i}, "CCSMST1"); take = 'C16orf91';
    elseif strcmp(Receptor{i}, "CIROP"); take = 'LMLN2';
    elseif strcmp(Receptor{i}, "FCGR1BP"); take = 'FCGR1B';
    elseif strcmp(Receptor{i}, "HIDE1"); take = 'C19orf38';
    elseif strcmp(Receptor{i}, "KCT2"); take = 'C5orf15';
    elseif strcmp(Receptor{i}, "KIR2DL2"); take = 'KIR3DL2';
    else; take = Receptor{i};
    end
    try TEMP = webread(['https://www.proteinatlas.org/api/search_download.php?search=' take '&format=json&columns=' HTTP{1} '&compress=no'], weboptions('Timeout', 60));
    catch; disp(i);
    end
    if numel(TEMP)
        TEMP(1).Gene = Receptor{i};
        SHORT = [SHORT; TEMP(1, :)];
    else
        disp(Receptor{i});
    end
end
save([TXT{1} '_SHORT'], 'SHORT');
writetable(struct2table(SHORT), [TXT{1} '_SHORT']);
toc; 
...
save([TXT{1} '_LONG'], 'LONG');
writecell(LONG, [TXT{1} '_LONG']);
toc;
...
figure(1); clf;
tightsubplot(1, 1, [0 0], [LOW 0.01],  [0.042 0.04])
set(gcf, 'color', 'w', 'Position', [0 0 1670 800]);
MAT = MAT(MAT(:, 3) > 1, :);
G = groupsummary(MAT, MAT(:, 1), "mean");
N = histcounts(MAT(:, 1), 0.5:1:size(G, 1)+0.5);
[~, I] = sort(G(:, 3));
CAT = regexprep(LONG(:, 1), '([A-Z])', ' $1');
CAT = strrep(CAT, '1', ' ');
CAT = strrep(CAT, '_', ' ');
...
ylabel('#Receptors')
ylim([0 YMAX1])
set(gca, 'FontSize', 20, 'YTick', 0:YSEP1:YMAX1, 'YColor', [72 40 120]/255);
...
set(gca, 'fontname', 'sansserif', 'FontSize', 19, 'YTick', 0:YSEP2:YMAX2, 'YColor',[72 40 120]/255);
saveas(gcf, [TXT{1} '_figure'], 'epsc')
f = gcf; f.PaperSize = [f.PaperPosition(3) f.PaperPosition(4)];
print([TXT{1} '_figure.pdf'], '-dpdf');
saveas(gcf, [TXT{1} '_figure'], 'svg');

tak = sortrows(G, 3);
[~, p] = kstest2(tak(1:10, 3), tak(end-10:end, 3));
writematrix(tak(:, 3), [TXT{1} '_meanEXP']);
toc;